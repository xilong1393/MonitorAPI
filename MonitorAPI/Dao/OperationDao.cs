...
namespace MonitorAPI.Dao
{
    public class OperationDao:BaseDao
    {
        ...
        private const string SQL_GETCLASSROOMSTATUS_CLASSROOMID1 = "SELECT CLASSROOM.CLASSROOMID, PPCREPORTTIME, ENGINEVERSION, ENGINESTATUS, AVSTATUS, AVCAPUTUREFRAMES, CAMERASTATUS, WBSTATUS,CLASSROOM.WBNumber as ClassRoomWBNumber, CLASSROOMENGINESTATUS.WBNUMBER, SGSTATUS, SDSTATUS, SSSTATUS, PPCIP, IPCIP, PPCCONNECTIONSTATUS, IPCCONNECTIONSTATUS, SVRPORTALPAGE, FREEDISK, COURSENAME, COURSEID, IPCREPORTTIME, AGENTVERSION, AGENTSTATUS, AGENTRECORDDATA, CLASSROOMNAME, CLASSROOMGROUPNAME FROM CLASSROOMGROUP, (CLASSROOM LEFT JOIN CLASSROOMENGINESTATUS ON CLASSROOM.CLASSROOMID=CLASSROOMENGINESTATUS.CLASSROOMID) LEFT JOIN CLASSROOMAGENTSTATUS ON CLASSROOM.CLASSROOMID=CLASSROOMAGENTSTATUS.CLASSROOMID WHERE CLASSROOM.CLASSROOMGROUPID=CLASSROOMGROUP.CLASSROOMGROUPID AND CLASSROOM.CLASSROOMID=@CLASSROOMID ORDER BY CLASSROOMNAME ASC";
        private const string SQL_GETCLASSROOMSTATUS_CLASSROOMID = "SELECT CLASSROOM.CLASSROOMID, PPCREPORTTIME, ENGINEVERSION, ENGINESTATUS, AVSTATUS, AVCAPUTUREFRAMES, CAMERASTATUS, WBSTATUS,CLASSROOM.WBNumber as ClassRoomWBNumber, CLASSROOMENGINESTATUS.WBNUMBER, SGSTATUS, SDSTATUS, SSSTATUS, PPCIP, IPCIP, PPCCONNECTIONSTATUS, IPCCONNECTIONSTATUS, SVRPORTALPAGE, FREEDISK, COURSENAME, COURSEID, IPCREPORTTIME, AGENTVERSION, AGENTSTATUS, AGENTRECORDDATA, CLASSROOMNAME, CLASSROOMGROUPNAME, ClassSchedule.ClassTypeID FROM CLASSROOMGROUP, (CLASSROOM LEFT JOIN CLASSROOMENGINESTATUS ON CLASSROOM.CLASSROOMID=CLASSROOMENGINESTATUS.CLASSROOMID) LEFT JOIN CLASSROOMAGENTSTATUS ON CLASSROOM.CLASSROOMID=CLASSROOMAGENTSTATUS.CLASSROOMID left outer join ClassSchedule on ClassroomEngineStatus.CourseID=ClassSchedule.ClassID WHERE CLASSROOM.CLASSROOMGROUPID=CLASSROOMGROUP.CLASSROOMGROUPID AND CLASSROOM.CLASSROOMID=@CLASSROOMID ORDER BY CLASSROOMNAME ASC";
        ...
        public ClassroomEngineAgentStatus GetClassroomStatusbyClassroomID(int classroomID)
        {
            using (SqlCommand command = new SqlCommand())
            {
                command.Connection = Connection;
                command.CommandType = CommandType.Text;
                command.CommandText = SQL_GETCLASSROOMSTATUS_CLASSROOMID;
                command.Parameters.AddWithValue("@CLASSROOMID", classroomID);
                ClassroomEngineAgentStatus result = SqlHelper.ExecuteReaderCmdObject<ClassroomEngineAgentStatus>(command);
                return result;
            }
        }
